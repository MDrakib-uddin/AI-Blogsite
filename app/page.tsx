      ...
      <header className="container mx-auto py-6">
        ...
        {menuOpen && (
          <nav className="flex flex-col md:hidden bg-gray-900 rounded-lg mt-4 px-4 py-3 space-y-2 shadow-lg z-50">
            ...
            {user && user.email === "rakibuddinraki2003@gmail.com" && (
              <Link href="/admin/dashboard" className="text-gray-200 hover:text-white transition-colors" onClick={() => setMenuOpen(false)}>
                Admin Dashboard
              </Link>
            )}
            {user && (
              <Link href="/bookmarks/" className="text-gray-200 hover:text-white transition-colors" onClick={() => setMenuOpen(false)}>
                Bookmarks
              </Link>
            )}
            {user ? (
